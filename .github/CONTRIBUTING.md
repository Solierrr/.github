...
> *Toda variável de ambiente (credencial, config, chave de terceiro...) da organização deve ser centralizada no nosso Vault, estamos utilizando o [`Infisical`](https://infisical.com). O `.env.example` de cada repositório só documenta os nomes esperados; o valor real sempre vem do Infisical.*

- _"Não sabe como utilizar?"_

Utilize nossa documentação oficial com orientações de como extrair um `.env` e adicionar novas variáveis ao projeto em 
